...
impl Assembler {
    ...
    pub(crate) fn is_fully_read(&self) -> bool {
        if let State::Unordered { ref recvd } = self.state {
            ...
            // explicitly. `RangeSet` is guaranteed to be contiguous, and `limit` is always equal to
            // the end of the highest range, so there's data we haven't received below `limit` iff...
            if recvd.len() > 1 || recvd.peek_min().map_or(false, |x| x.start != 0) {
                return false;
            }
        }
        // For unordered reads, we know that all data below `limit` has been received, so we just
        // need to check whether the application has read it. For ordered reads, any data we haven't
        // received below `limit` is necessarily followed by still-buffered data that we *have*
        // received which was responsible for increasing `limit`, because such data cannot be
